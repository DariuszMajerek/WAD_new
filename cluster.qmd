...
![](images/dbscan_metryki.png){fig-width="70%" width="700"}
...
Niech $X = \{x_1, x_2, \dots, x_n\} \subset \mathbb{R}^p$ oznacza zbiór
obserwacji, a $d(\cdot,\cdot)$ — dowolną metrykę w tej przestrzeni (np.
euklidesową). Algorytm OPTICS (*Ordering Points To Identify the Clustering
Structure*) stanowi uogólnienie DBSCAN, którego celem nie jest bezpośrednie
wyznaczenie klastrów dla jednej skali gęstości, lecz rekonstrukcja struktury
klastrowej w szerokim zakresie gęstości.
...
Parametr $\varepsilon_{\max}$ ogranicza zakres lokalnych obliczeń, natomiast
$\text{MinPts}$ definiuje pojęcie lokalnej gęstości, analogicznie jak w DBSCAN.

1.  Maksymalne sąsiedztwo punktu

Dla każdego punktu $x \in X$ definiuje się jego maksymalne sąsiedztwo jako
$\mathcal{N}_{\varepsilon_{\max}}(x) = \{ y \in X : d(x,y) \le \varepsilon_{\max} \}$.
Jest to zbiór punktów branych pod uwagę przy wyznaczaniu lokalnych miar
gęstości oraz relacji osiągalności.

2.  Odległość rdzeniowa (*core-distance*)

Jeżeli punkt $x$ posiada w swoim sąsiedztwie co najmniej $\text{MinPts}$
punktów, to jego odległość rdzeniowa definiowana jest jako
...
\end{cases}
$$ gdzie $x_{(\text{MinPts})}$ oznacza $\text{MinPts}$-ty najbliższy punkt
względem $x$ w zbiorze $\mathcal{N}_{\varepsilon_{\max}}(x)$.

Wielkość $\text{core-dist}(x)$ stanowi lokalną miarę gęstości: im mniejsza jej
wartość, tym gęstszy jest obszar wokół punktu $x$.

![](images/core_dist.png)

3.  Odległość osiągalności (*reachability-distance*)

Dla punktu $y \in \mathcal{N}_{\varepsilon_{\max}}(x)$, gdzie $x$ jest punktem
rdzeniowym, definiuje się warunkową odległość osiągalności $$
\text{reach-dist}(y \mid x) = \max\{\text{core-dist}(x),\, d(x,y)\}.
$$

Wartość ta opisuje minimalną skalę odległości, przy której punkt $y$ może
zostać „dołączony” do struktury gęstości reprezentowanej przez punkt rdzeniowy
$x$. Łączy ona informację o lokalnej gęstości (poprzez $\text{core-dist}(x)$)
oraz o faktycznej odległości geometrycznej. W algorytmie OPTICS każdemu
punktowi $y$ przypisywana jest jedna aktualna wartość $\text{reach-dist}(y)$,
rozumiana jako minimum po wszystkich dotychczas rozważanych punktach
rdzeniowych, z których $y$ jest osiągalny.

4.  Kolejka priorytetowa (*SeedList*)

W trakcie działania algorytmu utrzymywana jest kolejka priorytetowa *SeedList*,
zawierająca punkty nieodwiedzone, które są osiągalne z już przetworzonych
punktów rdzeniowych. Priorytetem punktu $y$ w tej kolejce jest jego aktualna
wartość $\text{reach-dist}(y)$. Mechanizm ten powoduje, że algorytm eksploruje
przestrzeń danych w kolejności rosnącej odległości osiągalności, czyli od
regionów najbardziej gęstych do coraz rzadszych.

5.  Kolejność przetwarzania punktów

Algorytm działa iteracyjnie według następującego schematu:

-   Jeśli *SeedList* jest pusta, wybiera się dowolny nieodwiedzony punkt
    $x_i \in X$ jako nowy punkt startowy.
...
-   Jeżeli $x_i$ jest punktem rdzeniowym, to:
    ...
    -   dla każdego nieodwiedzonego punktu
        $y \in \mathcal{N}_{\varepsilon_{\max}}(x_i)$ wyznacza się kandydacką
        wartość
        $\text{reach-dist}(y \mid x_i) = \max\{\text{core-dist}(x_i), d(x_i,y)\}$
    -   aktualizuje się
        $\text{reach-dist}(y) = \min\bigl(\text{reach-dist}(y),\ \text{reach-dist}(y \mid x_i)\bigr)$,
        przy czym dla punktów nowo napotkanych $\text{reach-dist}(y)$
        inicjalizuje się wartością $+\infty,$
    -   punkt $y$ umieszcza się (lub aktualizuje) w *SeedList* z priorytetem
        równym jego aktualnej wartości $\text{reach-dist}(y)$.
-   Jeżeli *SeedList* nie jest pusta, wybiera się z niej punkt o najmniejszej
    wartości $\text{reach-dist}$ i powtarza powyższy proces.
...
Na tej podstawie konstruuje się wykres osiągalności (*reachability plot*), gdzie
oś pozioma odpowiada kolejności punktów w *OrderList*, a oś pionowa wartościom
$R(x)$. Lokalne minima wykresu (doliny) odpowiadają klastrom o wysokiej gęstości,
natomiast wysokie wartości sygnalizują granice klastrów lub obszary szumu.

7.  Relacja z DBSCAN

Na podstawie wyników OPTICS można odtworzyć rozwiązania DBSCAN dla dowolnego
progu gęstości $\varepsilon \le \varepsilon_{\max}$, wybierając punkty
spełniające $C_k(\varepsilon) = { x \in X : R(x) \le \varepsilon }.$ W przeciwieństwie do DBSCAN, który generuje jeden podział danych dla ustalonej wartości $\varepsilon$, OPTICS ujawnia strukturę klastrów w sposób wieloskalowy,
umożliwiając analizę gęstości na ciągłym spektrum parametrów.
...
[^cluster-4]: Jeśli punkt $y$ nie był jeszcze w *SeedList*, to jego wartość
    $\text{reach-dist}(y)$ jest inicjalizowana na nieskończoność przed
    aktualizacją.
...
plot(cl$hc, main = "Nieskondensowane drzewo hierarchii gęstości (HDBSCAN*)")
...
Model mieszanek Gaussa (ang. *Gaussian Mixture Model*, GMM) jest jednym z
klasycznych probabilistycznych modeli klasteryzacyjnych. W przeciwieństwie do
metod geometrycznych, takich jak k-means czy DBSCAN, które dzielą przestrzeń na
obszary na podstawie odległości, GMM opisuje rozkład danych jako kombinację
(mieszaninę) wielu rozkładów normalnych. Dzięki temu umożliwia modelowanie
złożonych, nakładających się struktur oraz ocenę niepewności przypisań punktów
do klastrów.

1.  Idea modelu mieszanek Gaussa

Zakłada się, że populacja danych pochodzi z mieszaniny $K$ rozkładów
normalnych, z których każdy odpowiada jednemu klastrowi. Dla obserwacji
$x_i \in \mathbb{R}^d$ zakłada się $$
p(x_i) = \sum_{k=1}^{K} \pi_k \, \mathcal{N}(x_i \mid \mu_k, \Sigma_k)
$$ gdzie

-   $\pi_k$ – waga (udział) $k$-tej składowej, $\pi_k \ge 0$ i
    $\sum_{k=1}^{K} \pi_k = 1$,
-   $\mu_k$ – wektor średnich (centrum) $k$-tej składowej,
-   $\Sigma_k$ – macierz kowariancji $k$-tej składowej,
-   $\mathcal{N}(x_i \mid \mu_k, \Sigma_k)$ – gęstość wielowymiarowego rozkładu
    normalnego $$
    \mathcal{N}(x \mid \mu, \Sigma) = \frac{1}{(2\pi)^{d/2} |\Sigma|^{1/2}} \exp\!\left[-\frac{1}{2}(x-\mu)^\top \Sigma^{-1}(x-\mu)\right].
    $$ W ten sposób model GMM stanowi probabilistyczną wersję klasteryzacji, w
    której każdy punkt może należeć do kilku klastrów z określonymi
    prawdopodobieństwami.

2.  Zmienne ukryte i interpretacja probabilistyczna

Wprowadza się zmienną ukrytą $z_i \in \{1, \dots, K\}$, oznaczającą, z którego
składnika mieszaniny pochodzi obserwacja $x_i$. Model przyjmuje wtedy postać $$
P(x_i, z_i = k) = \pi_k \, \mathcal{N}(x_i \mid \mu_k, \Sigma_k).
$$ Dla każdego punktu oblicza się *posterior probability* (prawdopodobieństwo
...
$$ Wartość $\gamma_{ik}$ mieści się w przedziale $[0,1]$ i można ją
interpretować jako miękkie przypisanie punktu do klastra. W przeciwieństwie do
k-means, które wymusza jednoznaczne etykiety, GMM dopuszcza probabilistyczne
przypisania.
...
$$ Bezpośrednia maksymalizacja tej funkcji jest trudna, z tego powodu stosuje się algorytm EM (*Expectation–Maximization*),
który iteracyjnie przybliża rozwiązanie.

4.  Algorytm EM dla GMM

Algorytm EM (Dempster, Laird i Rubin, 1977) składa się z dwóch naprzemiennych
etapów:

-   Krok *Expectation* - obliczanie *posterior probabilities*
    (odpowiedzialności) dla każdego punktu i komponentu $$
    \gamma_{ik}^{(t)} =
    \frac{\pi_k^{(t)} \, \mathcal{N}(x_i \mid \mu_k^{(t)}, \Sigma_k^{(t)})}{
    \sum_{j=1}^{K} \pi_j^{(t)} \, \mathcal{N}(x_i \mid \mu_j^{(t)}, \Sigma_j^{(t)}) }.
    $$
-   Krok *Maximization* - aktualizacja parametrów mieszaniny, traktując
    $\gamma_{ik}$ jako wagi $$
...
Liczbę składników mieszaniny (liczbę klastrów) nie określa się z góry, lecz
dobiera na podstawie kryteriów informacyjnych, np.:

-   BIC (*Bayesian Information Criterion*) -
    $\mathrm{BIC} = -2 \log L_{\max} + p \log n$, gdzie $p$ to liczba
    parametrów modelu, $n$ liczba obserwacji. Najmniejsza wartość BIC wskazuje
    najlepszy kompromis między dopasowaniem a złożonością modelu.
-   AIC (*Akaike Information Criterion*) – która mniej penalizuje złożoność
    $\mathrm{AIC} = -2 \log L_{\max} + 2p.$

6.  Struktury kowariancji w GMM

Każdy komponent posiada macierz kowariancji $\Sigma_k$, która może mieć różne
ograniczenia:

-   *spherical* – $\Sigma_k = \sigma^2 I$ - klastry kuliste, identyczne
    wariancje,
-   *diagonal* – tylko wariancje na przekątnej, brak korelacji między cechami,
-   *ellipsoidal* – pełne macierze kowariancji (dowolne orientacje i
    rozciągnięcia).
...
-   Założenie o normalności – każdy klaster ma rozkład Gaussa, co bywa
    nieadekwatne dla struktur nieregularnych.
-   Wrażliwość na inicjalizację – EM może zbiec do lokalnego maksimum; często
    używa się wielu startów lub wstępnej inicjalizacji metodą k-means.
-   Brak odporności na odstające obserwacje – skrajne punkty mogą zaburzać
    estymację kowariancji.
-   Wymóg dodatnio określonych macierzy kowariancji – błędne dane lub
    współliniowość mogą prowadzić do problemów numerycznych.
