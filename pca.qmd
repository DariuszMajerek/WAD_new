...
Pierwsza składowa główna (`PC1`) jest kombinacją liniową wszystkich czterech zmiennych, przy czym trzy z nich — `Sepal.Length`, `Petal.Length` oraz `Petal.Width` — mają dodatnie ładunki, natomiast `Sepal.Width` ma ładunek ujemny. Oznacza to, że składowa ta rośnie, gdy długość działki kielicha oraz długość i szerokość płatków są duże, a maleje, gdy szerokość działki jest duża. Można zatem interpretować `PC1` jako wymiar opisujący ogólny rozmiar kwiatu: kwiaty o większych płatkach i węższych działkach uzyskują wyższe wartości tej składowej. W zbiorze `iris` `PC1` bardzo dobrze rozdziela gatunki: `setosa` ma wyraźnie niskie wartości tej składowej, `virginica` wysokie, a `versicolor` zajmuje pozycję pośrednią.

Druga składowa główna (`PC2`) ma inną strukturę ładunków. Jest zdominowana przede wszystkim przez silny ujemny współczynnik przy `Sepal.Width`, a wkład pozostałych zmiennych jest wyraźnie mniejszy. `PC2` opisuje więc głównie zróżnicowanie związane z szerokością działki kielicha i, w mniejszym stopniu, z jej długością. Ponieważ znak składowej jest konwencjonalny, najważniejsza jest tu interpretacja relatywna: obserwacje po przeciwnych stronach osi `PC2` różnią się przede wszystkim profilem cech działki kielicha, a nie rozmiarem płatków.

Interpretując wspólnie obie składowe, można stwierdzić, że `PC1` opisuje przede wszystkim wymiar wielkościowy kwiatu, natomiast `PC2` różnicuje obiekty głównie pod względem cech działki kielicha. W przestrzeni `PC1–PC2` `setosa` jest wyraźnie oddzielona od pozostałych gatunków głównie wzdłuż `PC1`, zaś rozdzielenie `versicolor` i `virginica` wynika z łącznego układu obu osi, a nie wyłącznie z `PC2`. Dwie pierwsze składowe wyjaśniają łącznie około 95.8% całkowitej wariancji, dlatego dają bardzo dobrą, choć nie doskonałą, reprezentację danych w dwóch wymiarach. Na potrzeby wizualizacji możemy narysować wykres *biplot* łączący obiekty i zmienne w przestrzeni dwóch pierwszych składowych.
