...
Formalna postać modelu eksploracyjnej analizy czynnikowej (EFA) zakłada, że zmienne obserwowalne $\mathbf{x} = (x_1, x_2, \ldots, x_p)^\top$ można wyrazić jako kombinację liniową czynników latentnych oraz składników specyficznych. Model przyjmuje postać [@introduc2020]:
...
### Metoda największej wiarygodności (ang. *Maximum Likelihood, ML*) [@lawley1940]
...
[^fa-3]: W metodzie największej wiarygodności estymacja macierzy ładunków czynnikowych $\Lambda$ oraz macierzy wariancji specyficznych $\Psi$ jest procedurą iteracyjną i wymaga ustalenia wartości startowych. W praktyce wartości te nie wynikają z jednej ścisłej reguły, lecz z podejścia zapewniającego stabilność i szybkie zbieganie algorytmu. W typowych implementacjach w R (np. w `stats::factanal` lub `psych::fa` z metodą ml) stosuje się kilka sprawdzonych metod inicjalizacji. Wartości początkowe macierzy $\Lambda$ ustala się najczęściej poprzez wykorzystanie wyników analizy głównych składowych. Ustalenie polega na wyznaczeniu wartości własnych i wektorów własnych macierzy korelacji, a następnie pobraniu pierwszych $m$ kolumn tej macierzy wektorów własnych jako punktu startowego. Zastosowanie PCA jako inicjalizacji wynika z tego, że wektory własne dobrze odzwierciedlają dominujące kierunki wariancji, co pozwala rozpocząć optymalizację w okolicy sensownego rozwiązania. Wartości początkowe macierzy $\Psi$ wyznacza się poprzez oszacowanie wariancji niewyjaśnionej przez czynniki wstępne. Standardowym zabiegiem jest przypisanie $\Psi$ jako macierzy diagonalnej, w której diagonalne elementy równe są różnicom pomiędzy jednością a komunalnościami wynikającymi z inicjalnych ładunków z PCA. Oznacza to, że jeżeli zmienna ma początkową komunalność równą $c$, to jej wariancja specyficzna w punkcie startowym wynosi $1 – c$. W niektórych implementacjach ustala się $\Psi$ jako macierz jednostkową, co jest rozwiązaniem neutralnym, jednak prowadzi zwykle do wolniejszej zbieżności.
...
-   Umożliwiają testy istotności liczby czynników:
...
-   Wymagają dużej próby i spełnienia założenia normalności wielowymiarowej.
...
bazującą na zmodyfikowanej macierzy korelacji, w której uwzględniono oszacowane komunalności, przy czym $\mathbf{Q}^*_m$ i $\mathbf{D}^*_m$ oznaczają zredukowane macierze $\mathbf{Q}$ i $\mathbf{D}$ do pierwszych $m$ kolumn odpowiadających pierwszym $m$ czynnikom.
...
-   **średnia arytmetyczna wartości bezwzględnych współczynników korelacji** danej zmiennej z innymi zmiennymi $$
    h_j^2=\frac{1}{p-1}\sum_{\substack{j'=1 \\ j'\ne j}}^p |r_{jj'}|
    $$
...
-   **współczynnik determinacji wielokrotnej** danej zmiennej z innymi zmiennymi (najczęściej stosowana i wykorzystywana przez `R`) $$
    h_j^2=R^2_{j\cdot 1,2,\ldots,j-1,j+1,\ldots,p},
    $$
...
-   **Dopasowanie do wariancji wspólnej** – PAF minimalizuje różnice pomiędzy macierzą zredukowanych korelacji $\mathbf{R}^*$ a aproksymacją $\Lambda \Lambda^\top$. Skupia się na wariancji wspólnej.
...
Metoda składowych głównych należy do klasy metod wspólnotowych, czyli takich, które zakładają klasyczny model czynnikowy
...
$$ W praktyce przyjęcie takiego założenia sprawia, że estymacja ładunków czynnikowych staje się równoznaczna z pobraniem wektorów własnych macierzy korelacji lub kowariancji, odpowiednio do liczby czynników. Oznacza to, że metoda *Principal Component Method* nie estymuje parametrów modelu czynnikowego w sensie statystycznym, lecz wykonuje dekompozycję macierzy korelacji i interpretuje jej wynik jako osadzenia czynnikowe. Oznacza to też, że macierz zredukowanych korelacji $\mathbf{R}^*$ jest po prostu zwykłą macierzą korelacji $\mathbf{R}$: $$
...
[^fa-4]: tu widać największą różnicę pomiędzy PCM a PAF; w metodzie PCM występuje jedna iteracja estymacji ładunków
...
-   **Prostota i szybkość** – metoda nie wymaga iteracji ani wartości startowych; ładunki wyznaczane są w jednym kroku na podstawie dekompozycji spektralnej macierzy korelacji.
...
-   **Powszechna dostępność** – metoda jest zaimplementowana w większości pakietów statystycznych i często stosowana jako punkt odniesienia.

#### Ograniczenia

-   **Przeszacowanie wariancji wspólnej** – przyjęcie $h_j^2=1$ oznacza, że cała wariancja zmiennej (łącznie z wariancją specyficzną i błędem) jest traktowana jako wspólna. Prowadzi to do zawyżenia ładunków czynnikowych w porównaniu z metodami estymującymi komunalności (PAF, ML).
...
-   **Niezgodność z modelem czynnikowym** – ponieważ PCM nie rozróżnia wariancji wspólnej i specyficznej ($\Psi = \mathbf{0}$), uzyskane ładunki nie odpowiadają ściśle modelowi $\Sigma = \Lambda\Lambda^\top + \Psi$. PCM jest raczej algebraiczną aproksymacją niż estymacją parametrów modelu czynnikowego.
...
F(\Lambda) = \sum_{i \neq j} \Big( r_{ij} - (\Lambda\Lambda^\top)_{ij} \Big)^2,
$$ gdzie:

-   $r_{ij}$ to element macierzy korelacji empirycznej $\mathbf{R}$,
-   $(\Lambda\Lambda^\top)_{ij}$ to element pozadiagonalny macierzy odtworzonej z modelu,
-   elementy diagonalne nie są uwzględniane, ponieważ komunalności traktowane są jako parametry uboczne (*nuisance parameters*), które nie niosą informacji o strukturze korelacji międzyzmiennowych.

Można to zapisać równoważnie jako $$
F(\Lambda) = \| \mathbf{R} - \Lambda \Lambda^\top \|^2_{off},
$$ gdzie $\|\cdot\|_{off}$ oznacza normę Frobeniusa liczoną tylko na elementach pozadiagonalnych macierzy. Macierz $\Psi$ nie występuje w kryterium, ponieważ jako macierz diagonalna nie wpływa na elementy poza przekątną.
...
3.  Szukamy takich ładunków $\Lambda$, które minimalizują sumę kwadratów elementów $\mathbf{U}$ poza przekątną. Realizowane jest to w dwóch krokach:

    -   Pierwszy krok polega na ustaleniu bieżących komunalności jako sum kwadratów ładunków dla każdej zmiennej, co zapewnia zgodność z ograniczeniem modelu. Te komunalności wprowadza się następnie do macierzy korelacji w celu uzyskania nowej macierzy zredukowanej.
    -   Drugi element obejmuje rozwiązanie problemu wartości własnych tej macierzy, które prowadzi do nowej macierzy osadzeń. Taki krok aktualizuje ładunki w sposób zmniejszający reszty pozadiagonalne bez użycia pochodnych funkcji $F$.

    W algorytmie MINRES nie wyznacza się gradientu funkcji $F$, lecz wykorzystuje się fakt, że macierz reszt można zredukować, dopasowując kierunki własne macierzy korelacji zredukowanej do struktury reszt. Aktualizacja ładunków jest więc realizowana przez wybór tych wektorów własnych, które najlepiej redukują różnice poza przekątną. Po każdej aktualizacji ponownie oblicza się macierz odtworzoną, wyznacza macierz reszt i sprawdza, czy zmiany funkcji $F$ są wystarczająco małe.
...
Estymacja metodą GLS przebiega poprzez iteracyjne aktualizowanie $\Lambda$ i $\Psi$. W pierwszej kolejności przyjmuje się początkową postać $\Psi$ (np. oszacowaną z wariancji resztowych lub metodą *principal factor/axis*). Następnie przekształca się zmienne przy użyciu macierzy $\Psi^{-1/2}$ w celu uzyskania tzw. zmiennych zważonych. Dla tak przekształconych danych stosuje się klasyczną metodę estymacji, ponieważ przekształcenie usuwa heteroskedastyczność składników specyficznych. Przykładowo, po przeprowadzeniu transformacji $Y = \Psi^{-1/2}X$, model przyjmuje postać

$$
Y = (\Psi^{-1/2}\Lambda)f+\Psi^{-1/2}\varepsilon,
$$ gdzie składnik losowy ma kowariancję równą macierzy jednostkowej. Ładunki czynnikowe szacujemy więc przez zwykłą metodę najmniejszych kwadratów dla zmiennych $Y$. Następnie uzyskane ładunki transformujemy z powrotem, otrzymując aktualizację $\Lambda$. Po tej aktualizacji wyznaczamy nową $\Psi$ i powtarzamy procedurę aż do zbieżności.

Metoda GLS pozwala włączać do analizy informację o strukturze wariancji specyficznych i ograniczać wpływ zmiennych o małej precyzji pomiaru. Takie podejście zapewnia lepszą stabilność estymacji i większą zgodność modelu z empiryczną strukturą kowariancji, zwłaszcza gdy zmienne charakteryzują się wyraźnie zróżnicowaną wariancją błędu.
...
::: callout-note
Istnieją również inne metody estymacji ładunków czynnikowych, jak metody bayesowskie [@lu2016], czy metody z regularyzacją LASSO ale nie są one częścią tego opracowania [@jacobucci2018].
:::
...
Proporcja wariancji wyjaśnionej przez model czynnikowy, czyli stosunek sumy wariancji wspólnej do całkowitej wariancji wszystkich zmiennych, stanowi podstawową miarę jakości dopasowania. W przypadku standaryzowanych zmiennych całkowita wariancja wynosi $p$, więc proporcja wariancji wyjaśniona przez model składający się z $m$ czynników ma postać $$
...
\Sigma=\Lambda\Lambda^\top + \Psi
$$ testujemy hipotezę $H_0$: model $\Sigma = \Lambda\Lambda^\top + \Psi$ jest poprawnie wyspecyfikowany w populacji. Statystyka $\chi^2$ rośnie wraz z pogarszającym się dopasowaniem (niestety duże próby sprzyjają odrzucaniu nawet dobrze dopasowanych modeli, a naruszenia normalności mogą zawyżać lub zaniżać wynik).
...
Analiza wartości własnych macierzy reszt uzupełnia powyższe podejścia. Po wyodrębnieniu $m$ czynników obliczamy resztową macierz korelacji $\mathbf{R}-\hat{\mathbf{R}}$ i zbadać jej wartości własne. Duże dodatnie wartości własne sygnalizują pozostawioną wspólną wariancję (niedomiar czynników) lub struktury lokalne.

Jakość lokalną rozwiązania ocenia się przez zasoby zmienności wspólnej i swoistej. $$
...
-   *Varimax* [@kaiser1958] - najczęściej stosowana rotacja ortogonalna. Maksymalizuje wariancję kwadratów ładunków w ramach każdego czynnika. Prowadzi do tego, że każda zmienna ma wysokie ładunki tylko na jednym czynniku, a bliskie zeru na pozostałych. Funkcja celu $$
...
-   *Oblimin* [@jennrich1966] - rodzina rotacji z parametrem $\gamma$, który reguluje stopień skośności. Dla $\gamma = 0$ rozwiązanie staje się *direct quartimin* (skośnym odpowiednikiem *quartimax*), a większe $\gamma$ prowadzą do większej korelacji czynników. Funkcja celu $$
    F(\Lambda^*) = \sum_{i=1}^p \sum_{j=1}^m \left(\lambda_{ij}^{*2} - \gamma \frac{\sum_{k=1}^m \lambda_{ik}^{*2}}{m}\right)^2.
    $$

-   *Promax* [@hendrickson1964] - rotacja skośna oparta na prostym podejściu dwustopniowym. Najpierw stosuje się rotację ortogonalną (najczęściej *varimax*), następnie ładunki są podnoszone do potęgi $k$ (zwykle 3 lub 4), aby wymusić prostą strukturę, i ponownie dopasowywane przy użyciu metody najmniejszych kwadratów $$
    \tilde{\lambda}_{jk} = \text{sign}(\lambda^*_{jk}) \cdot |\lambda^*_{jk}|^k.
    $$ Rotacja *promax* pozwala uzyskać bardziej realistyczne struktury, gdy czynniki są rzeczywiście skorelowane.

-   Geomin [@everitt1989] - minimalizuje średnią geometryczną kwadratów ładunków, co prowadzi do sytuacji, w której każda zmienna ma niewiele istotnych ładunków. Funkcja celu $$
    G(\Lambda^*) = \sum_{i=1}^p \left( \prod_{j=1}^m (\lambda_{ij}^{*2} + \epsilon) \right)^{1/m},
    $$ gdzie $\epsilon$ to mały parametr stabilizujący.

-   Simplimax [@kiers1994] - uogólnienie kryteriów upraszczających jak *varimax* czy *quartimax*, które minimalizuje liczbę dużych i małych ładunków w macierzy, pozwalając użytkownikowi sterować liczbą „prostych” elementów.
...
::: {#exm-1}
Na potrzeby ilustracji budowy modelu EFA wykorzystamy dane z pakietu `psych`, które zawierają wyniki różnych testów poznawczych [@horn1969]. Dane te są często używane jako przykład w literaturze dotyczącej analizy czynnikowej.
...
| ParagraphComprehension | Rozumienie tekstów pisanych | Zdolności werbalne |
...
Poniżej została przedstawiona macierz wartości bezwzględnych reszt. Jak widać warunek $|R-\hat{R}|<0{,}1$ jest spełniony dla prawie wszystkich reszt.
...
Choć wzrósł nieco poziom wyjaśnionej wariancji przez czynniki, to jednak rozwiązanie, w którym występują pojedyncze zmienne jako czynnik nie są pożądane. Dlatego pozostaniemy przy rozwiązaniu z czterema czynnikami.
:::

::: callout-tip
Liczba czynników, może być szacowana na podstawie różnych kryteriów, z których każde eksponuje inny aspekt, ale ostateczna decyzja o wyborze liczby czynników powinna być podyktowana głównie zgodnością otrzymanych wyników z teorią oraz interpretowalnością. Oczywiście nie powinno się to dziać kosztem znacznego obniżenia poziomu dopasowania modelu.
:::

Na temat konfirmacyjnej analizy czynnikowej (CFA) zostanie poświęcony kolejny rozdział.
